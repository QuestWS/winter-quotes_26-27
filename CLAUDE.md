...
- **`capture` attribute on a file input forces the camera** on Android, killing
  the gallery option. Only the dedicated camera buttons carry it, never the
  gallery input. Two further traps come with it, and both shipped once:
  `capture` cannot say *which mode* to open, so an `accept` naming both
  `image/*` and `video/*` is ambiguous and the browser answers by showing the
  gallery instead — stills and video need **one camera input each**. And
  `multiple` alongside `capture` makes Chrome on Android drop the capture
  entirely, so a camera input never takes more than one file.
...
| Excel import of last year's selections | **Built — one file at a time** | `Load from an old sheet` in the console; `check-legacy-import.js` covers all three file states. Reads the season folder off Drive, recovers the `#REF!` files from the master grid, prices at today's rates. Nothing marks which files are already imported, and a comparison sheet still needs a human to say which option was taken. |
