...
| `yard/index.html` | ~900 | The yard app: four lists, one unit at a time, the yard log, dictation, photos and re-measuring. One big `<script>`, same no-duplicate-names rule as the console |
...
| Moving a unit along (pulled / dropped off / stored) | `YARD_STATES_`, `adminSetYardState`, `yardStateOf_` | `markState`, `renderState`, `act_` (pulled is on the opened unit, not the row; dropped off is a row action on Awaiting and works from the console too) |
| Search and sort in the yard | — | `search_`, `storeList_`, `toggleSort`, `byLocation_`, `byName_` |
