...
| Full-size uploads that survive the yard | `adminUploadSession` | `putDirect_`, `upAsk_`, `upResume_`, `upWake_` |
...
| `check-yard-uploads.js` | Full-size yard uploads resume after a drop, wait out no signal, survive the app closing, and land byte-for-byte exactly once |
