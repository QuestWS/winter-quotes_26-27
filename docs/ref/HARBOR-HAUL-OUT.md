...
QW-26-1991 was paid in full, signed, and sitting in a storage tab, and the app
had never heard of it: no slip number, nothing recorded, so `listOf_`
...
harbor reads the name and says *that one's in B-12*. That is a correction the old
...
`d.placement`. One field plus the slip decides the list, so a unit can never be on two at
once or fall off all four — `check-harbor-haul-out.js` walks every
...
The "fall off all four" half of that assertion is not theoretical: it is the
bug Awaiting was added to fix, and it is checked rather than reasoned about for
that reason.
...
- **Dropped off is recordable from both surfaces**, and it is a row action on
  ...
  *"it doesn't hurt to have other people be able to contribute what they know."*
  ...
  not a decision, it is an observation, and the harbor and the lot see plenty of them.
...
`check-harbor-haul-out.js` asserts all of it from the rendered markup rather than from
