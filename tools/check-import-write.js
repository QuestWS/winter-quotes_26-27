...
{
  /* The write moved into importApplyCore_, shared by the one-at-a-time
     importer and the bulk run; adminImportApply is now a two-line wrapper. */
  const body = (gas.match(/^function importApplyCore_[\s\S]*?\n}/m) || [''])[0];
  check('importApplyCore_ exists', body.length > 0);
  check('adminImportApply goes through it',
    /importApplyCore_\(/.test((gas.match(/^function adminImportApply[\s\S]*?\n}/m) || [''])[0]));
