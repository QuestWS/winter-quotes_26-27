...
console.log('=== page and server share the one stamp ===');
{
  ...
  ['adminImportApply', 'adminRepriceApply'].forEach(name => {
    const body = fn(name);
...
{
  const body = fn('adminImportApply');
  if (!/COL\.REM\)\.setValue\(IMPORT_HOLD_MARK/.test(body)) {
    fail('adminImportApply does not write the reminder hold — imports would be auto-emailed');
